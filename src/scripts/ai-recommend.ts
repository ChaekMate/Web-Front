...
// 메시지 추가 함수
const addMessage = (type: 'user' | 'ai', text: string, books?: Book[]): void => {
...
// 로딩 메시지 추가
const addLoadingMessage = (): HTMLElement => {
  const chatMessages = document.getElementById('chatMessages');
...
// AI 추천 API 호출
const getRecommendation = async (message: string): Promise<RecommendResponse | null> => {
  try {
