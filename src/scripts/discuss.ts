...
// ==================== 도서 정보 로드 (더미 데이터) ====================
function loadBookInfo(bookId: number | null): void {
...
// ==================== 도서 정보 렌더링 ====================
function renderBookInfo(book: Book): void {
    ...
    if (bookCover) bookCover.src = book.cover_image;
    if (bookTitle) bookTitle.textContent = book.title;
    if (bookAuthor) bookAuthor.textContent = book.author.split('^')[0] + ' 외';
    if (bookPublisher) bookPublisher.textContent = book.publisher;
    if (bookPrice) bookPrice.textContent = book.price.toLocaleString() + '원';
    if (bookRating) bookRating.textContent = book.rating.toString();
...
// ==================== 추천 질문 클릭 ====================
function initSuggestedQuestions(): void {
