...
// URL 파라미터에서 도서 ID 추출
function getBookIdsFromUrl(): number[] {
...
// 도서 비교 API 호출
async function compareBooks(bookIds: number[]): Promise<void> {
    const loadingState = document.getElementById('loadingState');
    const errorState = document.getElementById('errorState');
    const compareContainer = document.getElementById('compareContainer');
    
    if (!loadingState || !errorState || !compareContainer) return;
    ...
    try {
        ...
        const summary = data.data?.comparison_summary || data.comparison_summary;
...
// 비교 결과 렌더링
function renderComparison(books: Book[], summary: CompareResponse['data']['comparison_summary']): void {
    renderSummary(summary);
    renderTable(books, summary);
}

// 비교 요약 렌더링
function renderSummary(summary: CompareResponse['data']['comparison_summary']): void {
...
// 비교 테이블 렌더링 (카드형)
function renderTable(books: Book[], summary: any): void {
    ...
    const cardsHtml = books.map(book => {
        ...
        if (isLowestPrice) cardClass += ' best-price';
        if (isHighestRating) cardClass += ' best-rating';
        
        const badges = [];
        if (isLowestPrice) badges.push('<span class="badge badge-price">최저가</span>');
        if (isHighestRating) badges.push('<span class="badge badge-rating">최고평점</span>');
