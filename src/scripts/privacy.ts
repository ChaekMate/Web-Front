/**
 * ChaekMate Privacy Policy TypeScript
 * 개인정보처리방침 페이지 기능 관리
 */

console.log('🔒 ChaekMate Privacy 로드 완료!');
...
function initSmoothScroll(): void {
    const navItems = document.querySelectorAll('.privacy-nav-item');
        ...
        (entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    const activeNav = document.querySelector(`.privacy-nav-item[href="#${id}"]`);
                    if (activeNav) {
    ...
    if (hash && hash.length > 1) {
        setTimeout(() => {
            ...
            if (targetElement) {
                ...
                const activeNav = document.querySelector(`.privacy-nav-item[href="${hash}"]`);
                if (activeNav) {
                    activeNav.classList.add('active');
                }
...
function initCopyText(): void {
    ...
    if (privacyContent) {
        privacyContent.addEventListener('copy', () => {
            console.log('개인정보처리방침 내용 복사됨');
        });
    }
...
// ==================== 메인 초기화 ====================
function initPrivacy(): void {
    console.log('🎬 ChaekMate Privacy 초기화 시작...');
    ...
    console.log('✨ ChaekMate Privacy 초기화 완료!');
}
