...
function setCategoryInfo() {
    ...
    const categoryInfo = (categories[category] || categories['novel']);
...
// ✅ 대안: 이벤트 위임 방식 (더 효율적)
function initBookCardClickDelegation() {
    const bookGrid = document.getElementById('bookGrid');
    if (!bookGrid)
        return;
    bookGrid.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('quick-view-btn')) {
            ...
            const bookId = bookCard === null || bookCard === void 0 ? void 0 : bookCard.getAttribute('data-book-id');
            console.log('미리보기:', bookId);
            alert('미리보기 기능은 준비 중입니다.');
            return;
        }
